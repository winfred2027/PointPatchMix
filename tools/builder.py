...
from datasets.modelnetdata import ModelNet40Attention
from datasets.scanobjectnndata import ScanObjectNNAttention
from datasets.scanobjectnnharddata import ScanObjectNNhardAttention
from datasets.scanobjectnnbgdata import ScanObjectNNbgAttention
from datasets.fewshotdata import FewShotAttention
...
def dataset_builder_score(args, config):
    dataset_args = {
        "split": config.train.others.subset,
        "data_path": config.train.data_path
    }
    if config.name == 'Modelnet40':
        dataset = ModelNet40Attention(**dataset_args)
    elif config.name == 'ScanObjectNN':
        dataset = ScanObjectNNAttention(**dataset_args)
    elif config.name == 'ScanObjectNNhard':
        dataset = ScanObjectNNhardAttention(**dataset_args)
    elif config.name == 'ScanObjectNNbg':
        dataset = ScanObjectNNbgAttention(**dataset_args)
    elif config.name == 'Modelnet40FewShot':
        dataset_args = {
            "split": config.train.others.subset,
            "data_path": config.train.data_path,
            "way" : config.train.others.way,
            "shot" : config.train.others.shot,
            "fold" : config.train.others.fold
        }
        dataset = FewShotAttention(**dataset_args)
    shuffle = config.train.others.subset == 'train'
    if args.distributed:
        sampler = torch.utils.data.distributed.DistributedSampler(dataset, shuffle=shuffle)
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=config.train.others.bs,
                                                 num_workers=int(args.num_workers),
                                                 drop_last=config.train.others.subset == 'train',
                                                 worker_init_fn=worker_init_fn,
                                                 sampler=sampler)
    else:
        sampler = None
...
def build_opti_sche(base_model, config):
    ...
    if sche_config.type == 'LambdaLR':
        ...
        scheduler = CosineLRScheduler(optimizer,
                ...
                cycle_limit=1,
                t_in_epochs=True)
    elif sche_config.type == 'ReduceLR':
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            mode='min',
            factor=sche_config.kwargs.decay_factor,
            patience=sche_config.kwargs.reduce_patience,
            verbose=True,
            min_lr=sche_config.kwargs.clip)
    elif sche_config.type == 'StepLR':
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, **sche_config.kwargs)
    elif sche_config.type == 'CosAnLR':
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, **sche_config.kwargs)
