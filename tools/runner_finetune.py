...
test_transforms = transforms.Compose(
    [
        #data_transforms.PointcloudJitter(std=0.01),
        #data_transforms.PointcloudJitter(std=0.03),
        #data_transforms.PointcloudJitter(std=0.05),
        #data_transforms.PointcloudRotateX(),
        #data_transforms.PointcloudRotateY(),
        #data_transforms.PointcloudRotateZ(),
        #data_transforms.PointcloudScale(scale_low=0.6, scale_high=0.6),
        #data_transforms.PointcloudScale(scale_low=2.0, scale_high=2.0),
        #data_transforms.PointcloudRandomInputDropout(),
        data_transforms.PointcloudScaleAndTranslate(),
    ]
)
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if (config.model.NAME == 'MixPointTransformer'):
                ret = base_model(points)
            else:
                points = points.transpose(2, 1)
                ret, trans_feat = base_model(points)

            _loss, acc = base_model.module.get_loss_acc(ret, label)
