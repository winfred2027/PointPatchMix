...
class PointcloudRotateX(object):
    def __call__(self, pc):
        bsize = pc.size()[0]
        for i in range(bsize):
            rotation_angle = np.random.uniform(low=-1./6., high=1./6.) * np.pi
...
class PointcloudRotateY(object):
    def __call__(self, pc):
        bsize = pc.size()[0]
        for i in range(bsize):
            rotation_angle = np.random.uniform(low=-1./6., high=1./6.) * np.pi
...
class PointcloudJitter(object):
    def __init__(self, std=0.03, clip=0.05):
...
class PointcloudScale(object):
    def __init__(self, scale_low=0.6, scale_high=0.6):
        self.scale_low = scale_low
        self.scale_high = scale_high
...
class PointcloudRandomInputDropout(object):
    ...
    def __call__(self, pc):
        bsize = pc.size()[0]
        for i in range(bsize):
            dropout_ratio = self.max_dropout_ratio  # 0.2
